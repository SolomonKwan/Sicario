...
NodeInfo::NodeInfo(Searcher* searcher) {
	...
	this->value = (this->turn == WHITE ? 1 : -1) *
			(Evaluator::psqtEvaluation(searcher->position) + Evaluator::getPieceEvaluation(searcher->position)) / 100; // NOTE currently an arbitrary divide by 100 in a crude attempt at normalisation.
}
